...
from util.load_data import load_ml_1m, build_matrix
...
class BayesianMatrixFactorization(Base):
    def __init__(self, num_user, num_item, num_feature, train, validation, **params):
        ...
        self.matrix = build_matrix(num_user, num_item, train)
    ...
    def _update_item_params(self):
        ...
        WI_post = self.WI_item + N * S_bar + np.dot(norm_X_bar, norm_X_bar.T) * (N * self.beta_item) / (self.beta_item + N) 
        ...
        # update mu_item
        mu_temp = (self.beta_item * self.mu_item + N * X_bar) / (self.beta_item + N)
    ...
    def _update_user_params(self):
        ...
        WI_post = self.WI_user + N * S_bar + np.dot(norm_X_bar, norm_X_bar.T) * (N * self.beta_user) / (self.beta_user + N) 
        #print 'WI_post', WI_post.shape
        ...
        # update mu_item
        mu_temp = (self.beta_user * self.mu_user + N * X_bar) / (self.beta_user + N)
        #print 'mu_temp', mu_temp.shape
    ...
    def _udpate_item_features(self):
        ...
        for item_id in xrange(self.num_item):
            ...
            covar = inv(self.alpha_item + self.beta * np.dot(features.T, features))
            #print 'covar', covar.shape
            lam = cholesky(covar)

            temp = self.beta * np.dot(features.T, rating) + np.dot(self.alpha_item, self.mu_item)
    ...
    def _update_user_features(self):
        ...
        for user_id in xrange(self.num_user):
            ...
            lam = cholesky(covar)
            temp = self.beta * np.dot(features.T, rating) + np.dot(self.alpha_user, self.mu_user)
            mean = np.dot(covar, temp)
            #print 'mean', mean.shape
            temp_feature = mean + np.dot(lam, rand.randn(self.num_feature, 1))
            temp_feature = np.reshape(temp_feature, (self.num_feature,))
...
def test():
    ...
    # params
    num_feature = 10
    model = BayesianMatrixFactorization(num_user, num_item, num_feature, train, validation, max_rating=5, min_rating=1)
    model.estimate()
