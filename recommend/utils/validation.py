...
def check_ratings(ratings, n_user, n_item, max_rating=None, min_rating=None):
    ...
    'ratings' must be a matrix with shape (n_sample, 3) and each row is (user_id, item_id, rating).
    ...
    if ratings.shape[1] != 3:
        raise DimensionError("Invalid rating format. number of column must be 3")
